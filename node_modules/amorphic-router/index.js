AmorphicRouter =
{
    ...
    */
    ...
    {
        if (route.load)
            this.location.search = this._encodeURL(route);
        else if (this.hasPushState)
            this.history.pushState({router: this, route: route}, this._encodeURL(route));
        else
    ...
    _checkURL: function()
    {
        ...
        if (this.location.pathname)
            parsed = this._parseURL(this.location.pathname + '?' + this.location.search);
        if (this.location.hash)
            parsed = this._parseURL(decodeURIComponent(this.location.hash), parsed);
    ...
    _parseURL: function(str, parsed)
    {
        ...
        if (parts.length > 1) {
            ...
            for (var ix = 0; ix < pairs.length; ++ix) {
                ...
                parsed.parameters[keyValue[0]] = keyValue.length > 1 ? keyValue[1] : '';
            }
        }
        return parsed;
    },
    ...
    _encodeURL: function (route)
    {
        ...
        for (var key in route.__parameters) {
            if (!(route.__parameters[key].encode === false)) {
                url += separator + key + '=' + this.controller.bindGet(route.__parameters[key].bind);
                separator = '&';
            }
        }
        return url;
