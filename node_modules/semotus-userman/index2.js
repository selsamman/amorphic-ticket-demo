module.exports.userman_mixins = function (objectTemplate, requires, mixinConfig, nconf)
{
    ...
    var privileges = mixinConfig.priveleges || [];
    var roles = mixinConfig.roles || ["user", "admin"];
    var roleDescriptions = mixinConfig.roleDescriptions ||  {
        "user": "User",             // A normal user
        "admin": "Administrator"   // An administrative user
    };
    var defaultUserRole = mixinConfig.defaultUserRole || "user";
    var defaultAdminRole = mxinConfig.defaultAdminRole || "admin";
    var defaultUserId = mixinConfig.defaultUserId || "root"
    var defaultPassword = mixinConfig.defaultPassword || "root";
    ...
    var SecurityRole = objectTemplate.create("person:SecurityRole",
    {
        name: {type: String},
    ...
    var SecurityContext = objectTemplate.create(
    {
        ...
        roles:          {toServer: false, type: Array, of: SecurityRole},
        roleValues:         function ()     {return roles},
        roleDescriptions:   function ()     {return roleDescriptions},
    ...
    Principal.mixin(
    {
         ...
         */
        establishPassword: function (password)
        {
        ...
        /*
         * Verify the email code passed in and reset the person record to allow registration to proceed
         */
        ...
        setPasswordChangeHash: function ()
        {
            ...
            return this.getSalt().then(function (salt) {
                ...
                this.passwordChangeExpires = new Date(((new Date()).getTime() +
                    (mixinConfig.passwordChangeExpiresHours || 24) * 60*60*1000));
                return this.persistSave();
    ...
    Controller.mixin(
    {
        ...
        publicRegister: {
            ...
            body: function (pageConfirmation, pageInstructions)
        {
            var person;
            ...
            return Principal.countFromPersistWithQuery({email: this.email}).then( function (count)
            {
                ...
                person = new Principal(this.email, this.firstName, "", this.lastName);
                return person.establishPassword(this.newPassword);

            }.bind(this)).then( function()
            {
                if (mixinConfig.validateEmail)
                    person.setEmailVerificationCode();
                else
                    this.setLoggedInState(person);

                this.sendEmail(mixinConfig.validateEmail ? "register_verify": "register",
                    person.email, this.firstName + " " + this.lastName, [
                        ...
                        {name: "link", content: url.protocol + "//" + url.host.replace(/:.*/, '') +
                            ...
                            "&code=" + person.validateEmailCode + "#verify_email"}
                    ]);
                if (mixinConfig.validateEmail && pageInstructions)
                    return this.setPage(pageInstructions);
                if (!mixinConfig.validateEmail && pageConfirmation)
            ...
            body: function(page)
        {
            ...
            return Principal.getFromPersistWithQuery({email: this.email}).then( function (persons)
            {
                ...
                return person.authenticate(this.password).then( function()
                {
                    this.setLoggedInState(person);
        ...
        */
        setLoggedInState: function (person)
        {
            this.loggedIn = true;
        ...
        */
        ...
        changeEmail: {
            ...
            body: function()
        {
            var oldEmail = this.person.email;
            ...
            return Principal.countFromPersistWithQuery({email: this.email}).then(function (count)
            {
                ...
                this.person.authenticate(this.password);

                this.person.email = newEmail;
                this.model.persistSave();
                
                this.sendEmail("email_changed", oldEmail, this.person.getFullName(), [
                    {name: "email", content: newEmail},
                    {name: "firstName", content: this.person.firstName}
                ]);
                
                this.sendEmail("email_changed", newEmail, this.person.getFullName(), [
                    {name: "email", content: newEmail},
                    {name: "firstName", content: this.person.firstName}
                ]);
        ...
        changePassword: {
            ...
            body: function(page)
        {
            return this.person.authenticate(this.password, true).then(function()
            {
                return this.person.establishPassword(this.newPassword).then(function ()
                {
                    log("Changed password for " + this.person.email);
            ...
            body: function(url, page)
        {
            ...
            return Principal.getFromPersistWithQuery({email: this.email}).then(function (persons)
            {
                ...
                return person.setPasswordChangeHash().then (function (token)
                {
                    this.sendEmail("password_reset",
                        this.email, person.firstName, [
                            {name: "link", content: url.protocol + "//" + url.host.replace(/:.*/, '') +
                                    (url.port > 1000 ? ':' + url.port : '') +
                                    "?email=" + encodeURIComponent(this.email) +
                                    "&token=" + token + "#reset_password"},
                            {name: "firstName", content: person.firstName}
                        ]);
         ...
         * Token was generated in publicResetPassword and kept in person entity to verify
         */
            ...
            body: function(page)
        {
            ...
            return Principal.getFromPersistWithQuery({email:this.email}).then(function (persons)
            {
                ...
                person = persons[0];
                return person.consumePasswordChangeToken(this.passwordChangeHash, this.newPassword);
        ...
        publicVerifyEmailFromCode: {on: "server", body: function(page)
        {
            ...
            return Principal.getFromPersistWithQuery({email:this.email}).then(function (persons)
            {
                ...
                person = persons[0];
                return this.consumeEmailVerificationCode(this.verifyEmailCode);
        ...
        checkDefaultUser: function () {
            return Principal.getFromPersistWithQuery({role: {name: defaultAdminRole}}).then(function (persons) {
                if (persons.length == 0) {
                    person = new Principal(defaultUserId, "Super", "", "User");
                    person.role = [defaultAdminRole];
                    return person.establishPassword(defaultPassword);
                }
            });
        }
    });
